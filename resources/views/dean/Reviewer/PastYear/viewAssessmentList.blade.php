...
function w3_close() {
  ...
  $(function () {
        ...
        if($('.search').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "{{$character}}/Reviewer/PastYear/assessment/list/searchAssessmentlist/",
...
<div id="all">
    <div>
        ...
        <p class="pass_page">
            ...
            <a href="{{$character}}/Reviewer/PastYear/assessment/{{$course[0]->course_id}}">Continuous Assessment</a>/
            <a href="{{$character}}/Reviewer/PastYear/assessment/{{$course[0]->course_id}}/assessment_name/{{$previous[0]->course_id}}">{{$previous[0]->semester_name}}</a>/
                      ...
                      @if($row_group->ass_type == $row->ass_type)
                        <div class="col-12 row align-self-center" id="course_list">
                          <div class="col-9 row align-self-center" >
                            ...
                            <a href="{{$character}}/Reviewer/PastYear/images/assessment/{{$course[0]->course_id}}-{{$row->ass_document}}" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="{{$previous[0]->semester_name}} : {{$assessments->assessment_name}} / {{$row_group->ass_type}} / {{$row->ass_name}} <br> <a href='{{$character}}/Reviewer/PastYear/assessment/view/whole_paper/{{$course[0]->course_id}}-{{$row->ass_id}}' class='full_question' target='_blank'>Whole paper</a>">

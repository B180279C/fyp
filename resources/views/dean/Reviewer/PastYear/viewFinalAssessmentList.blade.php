...
function w3_close() {
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.download_button', function(){
      ...
      window.location = "{{$character}}/Reviewer/PastYear/FinalAssessment/download/"+original_id+"-"+num[2];
    });
    ...
    $(document).on('click', '#checkDownloadAction', function(){
      ...
      if(checkedValue!=""){
        var fx_id = $('#fx_id').val();
        var original_id = $('#original_id').val();
        var id = original_id+"---"+fx_id+"---"+checkedValue;
...
<div id="all">
    <div>
        ...
        <p class="pass_page">
            ...
            <a href="{{$character}}/Reviewer/PastYear/FinalAssessment/{{$course[0]->course_id}}">Final Assessment</a>/
            <a href="{{$character}}/Reviewer/PastYear/FinalAssessment/{{$course[0]->course_id}}/assessment_name/{{$previous[0]->course_id}}">{{$previous[0]->semester_name}}</a>/
                ...
                <div id="action_sidebar" class="w3-animate-right" style="display: none">
                  ...
                  <ul class="sidebar-action-ul">
                      ...
                      <input type="hidden" id="fx_id" value="{{$AssFinal->fx_id}}">
                        ...
                        <a href='{{$character}}/PastYear/FinalAssessment/list/download/zipFiles/{{$course[0]->course_id}}---{{$AssFinal->fx_id}}/All'><li class="sidebar-action-li"><i class="fa fa-download" style="padding: 0px 10px;" aria-hidden="true"></i>All Result</li></a>
                  </ul>
                </div>
                <br>
                <br>
                      ...
                      @if($row_group->ass_fx_type == $row->ass_fx_type)
                        <div class="col-12 row align-self-center" id="course_list">
                          <div class="col-9 row align-self-center" >
                            ...
                            <a href="{{$character}}/Reviewer/PastYear/images/final_assessment/{{$course[0]->course_id}}-{{$row->ass_fx_document}}" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="{{$previous[0]->semester_name}} : {{$AssFinal->assessment_name}} / {{$row_group->ass_fx_type}} / {{$row->ass_fx_name}} <br> <a href='{{$character}}/Reviewer/PastYear/final_assessment/view/whole_paper/{{$course[0]->course_id}}-{{$row->fx_id}}' class='full_question' target='_blank'>Whole paper</a>">

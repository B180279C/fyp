...
<div id="all">
    <div>
        ...
        <p class="pass_page">
            ...
            @else
              <?php
                $page_title = $assessments[0]->semester_name." : ".$question;
              ?>
              <a href="/assessment/create/{{$course[0]->course_id}}/question/{{$question}}" class="first_page">Go Back</a>/
              <span class="now_page">{{$assessments[0]->semester_name}} : {{$question}}</span>/
            @endif
        </p>
            ...
            <div class="details" style="padding: 0px 5px 5px 5px;">
              ...
              @foreach($assessments as $row)
                ...
                <div class="col-12 row align-self-center" id="course_list">
                  @if($list=="list")
                    <a href="/assessment/folder/{{$course[0]->course_id}}/previous/{{$row->ass_id}}/list" id="show_image_link" class="col-9 row align-self-center">
                  @else
                    <a href="/assessment/folder/{{$course[0]->course_id}}/previous/{{$row->ass_id}}/once" id="show_image_link" class="col-9 row align-self-center">
                  @endif

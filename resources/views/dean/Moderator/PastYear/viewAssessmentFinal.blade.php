...
function w3_close() {
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.download_button', function(){
      ...
      window.location = "{{$character}}/Moderator/PastYear/FinalAssessment/download/"+course_id+"-"+num[2];
    });
    ...
    $(document).on('click', '#checkDownloadAction', function(){
      ...
      for(var i=0; inputElements[i]; i++){
        if(inputElements[i].checked){
          if(className==""){
            className = $(inputElements[i]).attr('class');
          }else{
            if($(inputElements[i]).attr('class')!=className){
              error = "error";
            }
          }
      ...
      if(checkedValue!=""&&error==""){
        ...
        if(className=="group_q group_download"){
          if($('.search').val()!=""){
            var data = $('#data').val();
            if(data=="name"){
              window.location = "{{$character}}/PastYear/FinalAssessment/download/zipFiles/"+id+"/searched";
            }else{
              window.location = "{{$character}}/PastYear/FinalAssessment/download/zipFiles/"+id+"/searchedWord";
            }
          }else{
            window.location = "{{$character}}/PastYear/FinalAssessment/download/zipFiles/"+id+"/checked";
          }
        }else{
          if($('.search_result').val()!=""){
            var data = $('#data').val();
            window.location = "{{$character}}/PastYear/FinalSampleResult/download/zipFiles/"+id+"/"+data;
          }else{
            window.location = "{{$character}}/PastYear/FinalSampleResult/download/zipFiles/"+id+"/checked";
          }
        }
      }else if(error=="error"){
  ...
  $(function () {
        ...
        if($('.search').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "{{$character}}/Moderator/PastYear/FinalAssessment/searchAssessment/",
        ...
        if($('.search_result').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "{{$character}}/Moderator/PastYear/FinalSampleResult/searchAssessmentResult/",
              data:{value:value,course_id:course_id},
              success:function(data){
    ...
    <div class="row" style="padding: 10px 10px 5px 10px;">
        <div class="col-md-12">
             ...
             <button onclick="w3_open()" class="button_open" id="button_open" style="float: right;margin-top: 10px;"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></button>
                <div id="action_sidebar" class="w3-animate-right" style="display: none">
                  ...
                  <ul class="sidebar-action-ul">
                      ...
                      <input type="hidden" id="course_id" value="{{$course[0]->course_id}}">
                        ...
                        <a href='{{$character}}/PastYear/FinalAssessment/download/zipFiles/{{$course[0]->course_id}}/All'><li class="sidebar-action-li"><i class="fa fa-download" style="padding: 0px 10px;" aria-hidden="true"></i>All Result</li></a>
                  </ul>
                </div>
                <br>
                <br>
              <h5 style="margin:-15px 0px 10px 10px;">Question & Solution</h5>
                  ...
                  <div class="row" id="assessments"  style="margin-top: -25px;">
                    ...
                    <div class="col-12 row align-self-center" id="course_list">
                      <div class="col-8 row align-self-center">
                        ...
                        <a href="{{$character}}/Moderator/PastYear/FinalAssessment/{{$course[0]->course_id}}/assessment_name/{{$row->course_id}}" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">
              ...
              <h5 style="margin:10px 0px 10px 10px;">Sample Result</h5>
              <div class="details" style="padding: 0px 5px;">
                  <div class="col-md-6 row" style="padding:0px 20px;position: relative;top:-25px;">
                      <div class="col-1 align-self-center" style="padding: 15px 0px 0px 2%;">
                          <p class="text-center align-self-center" style="margin: 0px;padding:0px;font-size: 20px;width: 30px!important;border-radius: 50%;background-color: #0d2f81;color: gold;">
                      ...
                      <div class="col-11" style="padding-left: 20px;">
                          <div class="form-group">
                              ...
                              <span class="tooltiptext_2">
                                ...
                                <hr style="background-color: #d9d9d9;margin: 3px 0px;">
                                <span>1. Semester</span><br/>
                                <span>2. Student Id OR Name</span><br/>
                                <span>3. Batch </span><br/>
                                <span>4. Submitted By ( Lecturer )</span><br/>
                              </span>
                          </div>
                      </div>
                  </div>
                  <div class="row" id="results"  style="margin-top: -25px;">
                    ...
                    <div class="col-12 row align-self-center" id="course_list">
                      <div class="col-8 row align-self-center">
                        ...
                        <a href="{{$character}}/Moderator/PastYear/FinalSampleResult/{{$course[0]->course_id}}/previous/{{$row->course_id}}/All" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">
                          <div class="col-1" style="position: relative;top: -2px;">
                            <img src="{{url('image/folder2.png')}}" width="25px" height="25px"/>
                          </div>
                          <div class="col-10" id="course_name">

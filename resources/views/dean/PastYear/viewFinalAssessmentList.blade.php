...
function w3_close() {
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '#checkDownloadAction', function(){
      ...
      if(checkedValue!=""){
        ...
        window.location = "/PastYear/FinalAssessment/list/download/zipFiles/"+id+"/checked";
      }else{
  ...
  $(function () {
        ...
        if($('.search').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "/PastYear/FinalAssessment/list/searchAssessmentlist/",
...
<div id="all">
    <div>
        ...
        <p class="pass_page">
            ...
            <a href="/PastYear/FinalAssessment/{{$course[0]->course_id}}">Final Assessment</a>/
            <a href="/PastYear/FinalAssessment/{{$course[0]->course_id}}/assessment_name/{{$previous[0]->course_id}}">{{$previous[0]->semester_name}}</a>/
                ...
                <div id="action_sidebar" class="w3-animate-right" style="display: none">
                  ...
                  <ul class="sidebar-action-ul">
                      ...
                      <input type="hidden" id="fx_id" value="{{$AssFinal->fx_id}}">
                        ...
                        <a href='/PastYear/FinalAssessment/list/download/zipFiles/{{$AssFinal->fx_id}}/All'><li class="sidebar-action-li"><i class="fa fa-download" style="padding: 0px 10px;" aria-hidden="true"></i>All Result</li></a>
                  </ul>
                </div>
                <br>
                <br>
                      ...
                      @if($row_group->ass_fx_type == $row->ass_fx_type)
                        <div class="col-12 row align-self-center" id="course_list">
                          <div class="col-9 row align-self-center" >
                            ...
                            <a href="/images/final_assessment/{{$row->ass_fx_document}}" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="{{$course[0]->semester_name}} : {{$AssFinal->assessment_name}} / {{$row_group->ass_fx_type}} / {{$row->ass_fx_name}} <br> <a href='/final_assessment/view/whole_paper/{{$row->fx_id}}' class='full_question' target='_blank'>Whole paper</a>">

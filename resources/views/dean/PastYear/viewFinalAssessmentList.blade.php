...
function w3_close() {
  ...
  $(function () {
        ...
        if($('.search').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "{{route($route_name.'.PY.final.searchAssessmentlist')}}",
                      ...
                      @if($row_group->ass_fx_type == $row->ass_fx_type)
                        <div class="col-12 row align-self-center" id="course_list">
                          <div class="col-9 row align-self-center" >
                            ...
                            <a href="{{$character}}/PastYear/images/final_assessment/{{$course[0]->course_id}}-{{$row->ass_fx_document}}" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="{{$previous[0]->semester_name}} : Final / {{$row_group->ass_fx_type}} / {{$row->ass_fx_name}} <br> <a href='{{$character}}/PastYear/final_assessment/view/whole_paper/{{$course[0]->course_id}}-{{$row->course_id}}' class='full_question' target='_blank'>Whole paper</a>">

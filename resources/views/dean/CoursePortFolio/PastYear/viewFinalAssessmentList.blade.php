...
function w3_close() {
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.download_button', function(){
      ...
      window.location = "{{$character}}/CourseList/PastYear/FinalAssessment/download/"+original_id+"-"+num[2];
    });
  ...
  $(function () {
        ...
        if($('.search').val()!=""){
          ...
          $.ajax({
              type:'POST',
              url: "{{$character}}/CourseList/PastYear/FinalAssessment/list/searchAssessmentlist/",
...
<div id="all">
    <div>
        ...
        <p class="pass_page">
            ...
            <a href="{{$character}}/CourseList/PastYear/FinalAssessment/{{$course[0]->course_id}}">Final Assessment</a>/
            <a href="{{$character}}/CourseList/PastYear/FinalAssessment/{{$course[0]->course_id}}/assessment_name/{{$previous[0]->course_id}}">{{$previous[0]->semester_name}}</a>/
                      ...
                      @if($row_group->ass_fx_type == $row->ass_fx_type)
                        <div class="col-12 row align-self-center" id="course_list">
                          <div class="col-9 row align-self-center" >
                            ...
                            <a href="{{$character}}/CourseList/PastYear/images/final_assessment/{{$course[0]->course_id}}-{{$row->ass_fx_document}}" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="{{$previous[0]->semester_name}} : {{$AssFinal->assessment_name}} / {{$row_group->ass_fx_type}} / {{$row->ass_fx_name}} <br> <a href='{{$character}}/CourseList/PastYear/final_assessment/view/whole_paper/{{$course[0]->course_id}}-{{$row->fx_id}}' class='full_question' target='_blank'>Whole paper</a>">

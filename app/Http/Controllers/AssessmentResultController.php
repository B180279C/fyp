   ...
   public function submissionFormEdit(Request $request)
    ...
    public function viewstudentlist($ass_rs_id)
    {
        ...
        $student_result = DB::table('assessment_result_students')
                 ...
                 ->select('assessment_result_students.*','students.*','users.*')
                 ->where('assessment_result_students.ass_rs_id', '=', $ass_rs_id)
                 ->where('assessment_result_students.submitted_by','=', 'Student')
                 ->where('assessment_result_students.status','=','Active')
                 ->groupBy('assessment_result_students.student_id')
                 ->get();
                ...
                foreach($group as $row_group){
                    ...
                    foreach($assessment_results as $row){
                        if($row_group->assessment == $row->assessment){
                            ...
                            $result .= '<a href="/AssessmentResult/studentResult/'.$row->ass_rs_id.'/" class="col-8 row align-self-center" id="show_image_link">';
    ...
    public function searchStudentList(Request $request)
    {
        ...
        if($value!=""){
            ...
            $result .= '<div class="row col-md-12 l_plus" style="border:0px solid black;margin: 0px;padding:0px;font-size: 20px;">';
            $result .= '<div class="col-md-3" style="padding-left: 18px;border:0px solid black" class="col-md-12">';
            $result .= 'Searched Result (<i class="fa fa-minus" aria-hidden="true" id="icon_l" style="color: #0d2f81;position: relative;top: 2px;"></i>)';
            $result .= '</div>';
            ...
            $result .= '<div class="row col-md-12" id="lecturer"  style="margin:12px 0px 0px 0px;padding: 0px;">';
                foreach($result_list as $row){
                    $result .= '<div class="row col-md-4 align-self-center" id="course_list" style="margin:0px 0px 5px 0px;">';
                    $result .= '<a href="/AssessmentResult/view/student/'.$row->ar_stu_id.'/" class="col-12 row align-self-center" id="show_image_link">';
                    $result .= '<div class="col-12 row" style="padding:10px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;">';
            ...
            $result .= '<div class="row col-md-12 l_plus" style="border:0px solid black;margin: 0px;padding:0px;font-size: 20px;">';
            $result .= '<div class="col-md-3" style="padding-left: 18px;border:0px solid black" class="col-md-12">';
            $result .= 'Submitted By Lecturer (<i class="fa fa-minus" aria-hidden="true" id="icon_l" style="color: #0d2f81;position: relative;top: 2px;"></i>)';
            $result .= '</div>';
            $result .= '<div class="col-9 show_count" style="border:0px solid black;">';
            $result .= '<hr style="display: inline-block; background-color: #cccccc;width: 94%;margin:0px;position: relative;top: -5px;">';
            ...
            if(count($student_result)>0) {
                $result .= '<div class="row col-md-12 s_plus" style="border:0px solid black;margin: 20px 0px 0px 0px;padding:0px;font-size: 20px;">';
                $result .= '<div class="col-md-3" style="padding-left: 18px;border:0px solid black" class="col-md-12">';
                $result .= 'Submitted By Students (<i class="fa fa-minus" aria-hidden="true" id="icon_s" style="color: #0d2f81;position: relative;top: 2px;"></i>)';
                $result .= '</div>';
                $result .= '<div class="col-9 show_count" style="border:0px solid black;">';
                $result .= '<hr style="display: inline-block; background-color: #cccccc;width: 94%;margin:0px;position: relative;top: -5px;">';
                ...
                $result .= '<div class="row col-md-12" id="student"  style="margin:12px 0px 0px 0px;padding: 0px;">';
            
                foreach($student_result as $sow){
                    $result .= '<div class="row col-md-4 align-self-center" id="course_list" style="margin:0px 0px 5px 0px;">';
                    $result .= '<a href="/AssessmentResult/view/student/'.$sow->ar_stu_id.'/" class="col-12 row align-self-center" id="show_image_link">';
                    $result .= '<div class="col-12 row" style="padding:10px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                    $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
                    $result .= '</div>';
    ...
    public function AllZipFileDownload($id)
    {
    ...
    public function zipFileDownload($ass_rs_id)
    {
        $group_result = DB::table('assessment_result_students')
                     ->join('students','students.student_id', '=', 'assessment_result_students.student_id')
                     ->join('users','users.user_id', '=', 'students.user_id')
    ...
    public function zipFileDownloadStudent($student_id,$ass_rs_id)
    {
        $assessment_result = AssessmentResult::where('ass_rs_id', '=', $ass_rs_id)->firstOrFail();
        $course_id = $assessment_result->course_id;

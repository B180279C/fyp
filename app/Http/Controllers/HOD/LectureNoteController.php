...
class LectureNoteController extends Controller
{
    public function viewLectureNote($id){
        ...
        if(count($course)>0){
            return view('hod.LectureNote.viewLectureNote',compact('course','lecture_note','previous_semester','all_note'));
        }else{
    ...
    public function folder_view($folder_id)
    {
        ...
        if(count($course)>0){
        	return view('hod.LectureNote.LectureNoteFolderView', compact('course','note_place','lecture_note','lecture_note_list','all_note','data'));
        }else{
            return redirect()->back();
        }
    }
	        ...
	        if(count($lecture_note)>0) {
	        	foreach($lecture_note as $row){
		        	...
		        	if($row->note_type=="folder"){
                        ...
                        $result .= '</div>';
                        $result .= '<a href="/hod/lectureNote/folder/'.$row->ln_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                        ...
                        if(($ext[1] == "pdf")||($ext[1] == "docx")||($ext[1] == "xlsx")||($ext[1] == "pptx")){
                            ...
                            $result .= '<input type="checkbox" value="'.$row->ln_id.'" class="group_download_list">';
                            $result .= '</div>';
                            $result .= '<a href="'.action('HOD\LectureNoteController@downloadLN',$row->ln_id).'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                            if($ext[1]=="pdf"){
                            ...
                            $result .= '<input type="checkbox" value="'.$row->ln_id.'" class="group_download_list">';
                            $result .= '</div>';
                            $result .= '<a href="/hod/images/lectureNote/'.$row->note.'" data-toggle="lightbox" data-gallery="example-gallery_student" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$row->note_name.$semester_name.'">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';

...
class PastYearNoteController extends Controller
{
	public function PastYearNote($id)
    {
        ...
        if(count($course)>0){
            return view('dean.PastYearNote.viewPYNote',compact('course','previous_semester'));
        }else{
	        ...
	        if(count($lecture_note)>0) {
	        	foreach($lecture_note as $row){
		        	...
		        	if($row->note_type=="folder"){
	                    ...
	                    $result .= '</div>';
	                    $result .= '<a href="/PastYearNote/'.$course_id.'/folder/'.$row->ln_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            ...
	            if(count($previous_semester)>0) {
	            	foreach($previous_semester as $row){
		                ...
		                $result .= '</div>';
		                $result .= '<a href="/PastYearNote/'.$course_id.'/course/'.$row->course_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		                $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		                $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
		                $result .= '</div>';
    ...
    public function zipFileDownload($course_id,$download){
        ...
        if($download == "All"){
        	...
        	for($i=1;$i<(count($string)-1);$i++){
        		$semester = DB::table('lecture_notes')
                    ->join('courses','courses.course_id','=','lecture_notes.course_id')
                    ->join('subjects','subjects.subject_id','=','courses.subject_id')
                    ->join('semesters','courses.semester','=','semesters.semester_id')
                    ->select('lecture_notes.*','courses.*','subjects.*','semesters.*')
                    ...
                    ->where('lecture_notes.status', '=', 'Active')
                    ->get();
                $zip->addEmptyDir($semester[0]->semester_name);
                	...
                	foreach ($files as $key => $value) {
		                ...
		                if($note->note==$relativeNameInZipFile){
		                    ...
		                    if($note->note_place=="Note"){
		                        $zip->addFile($value,$semester[0]->semester_name."/".$note->note_name.'.'.$ext[1]);
		                    }else{
		                        $m=1;
		                        ...
		                        $zip->addFile($value,$semester[0]->semester_name."/".$data.'/'.$note->note_name.'.'.$ext[1]);
		                    }
		                }
		            }
                }else{
		            $zip->addEmptyDir($semester[0]->semester_name."/".$note->note_name);

		            $check = $note->note_place.",,,".$note->ln_id;
		            $lecture_note = DB::table('lecture_notes')
				                    ->select('lecture_notes.*')
				                    ->where('note_place','=',$check)
				                    ->where('lecture_notes.status', '=', 'Active')
				                    ->orderByDesc('lecture_notes.note_type')
				                    ->get();
				    foreach($lecture_note as $row){
				    	if($row->note_type == "document"){
			                foreach ($files as $key => $value) {
			                    ...
			                    if($row->note==$relativeNameInZipFile){
			                        $ext = explode('.',$relativeNameInZipFile);
			                        if($row->note_place=="Note"){
			                            $zip->addFile($value,$semester[0]->semester_name."/".$row->note_name.'.'.$ext[1]);
			                        }else{
			                            $m=1;
			                            $place = explode(',,,',$row->note_place);
			                            $data = "";
			                            ...
			                            $zip->addFile($value,$semester[0]->semester_name."/".$data.'/'.$row->note_name.'.'.$ext[1]);
			                        }
			                    }
			                }
			            }else{
			                if($row->note_place=="Note"){
			                    $zip->addEmptyDir($semester[0]->semester_name."/".$row->note_name);
			                }else{
			                    $m=1;
			                    $place = explode(',,,',$row->note_place);
			                    $data = "";
			                    ...
			                    $zip->addEmptyDir($semester[0]->semester_name."/".$data.'/'.$row->note_name);
			                }
			            }
				    }
                }
    ...
    public function PastYearNoteViewIn($id,$view,$view_id)
    {
        ...
        if(count($course)>0){
        	if($view=="course"){
            	return view('dean.PastYearNote.viewPYNoteFolder',compact('course','lecture_note','previous'));
        	}else{
        		return view('dean.PastYearNote.viewPYNoteFolder',compact('course','lecture_note','previous','ln','data'));
        	}
        }else{
            return redirect()->back();
        }

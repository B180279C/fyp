...
class C_PastYearTPController extends Controller
{
  public function PastYearTP($id)
  {
    $user_id       = auth()->user()->user_id;
        ...
        if(count($course)>0){
            return view('dean.CoursePortFolio.PastYearTP.viewPYTP',compact('course','previous_semester'));
        }else{
        ...
        if($value!=""){
            if(count($previous_semester)>0) {
              ...
              $result .= '<input type="checkbox" value="'.$row->course_id.'" class="group_download">';
                $result .= '</div>';
                $result .= '<a href="'.$character.'/CourseList/PastYearTP/'.$course_id.'/course/'.$row->course_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';

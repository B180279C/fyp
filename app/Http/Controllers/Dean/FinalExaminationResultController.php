...
class FinalExaminationResultController extends Controller
{
    ...
    public function zipFileDownload($course_id,$download)
    {
        ...
        $ZipFile_name = "Final Result ( ".$subjects[0]->subject_code." )";
        $zip = new ZipArchive;
        $fileName = storage_path('private/Final_Result/Zip_Files/'.$ZipFile_name.'.zip');
        ...
        if($this->checkCoursePerson($f_course_id)==true){
            ...
            Storage::disk('private')->delete('/Final_Result/Zip_Files/'.$ZipFile_name.'.zip');
            return redirect()->back();
        }
    }

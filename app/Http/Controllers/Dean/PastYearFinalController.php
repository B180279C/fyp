...
class PastYearFinalController extends Controller
{
    ...
    public function zipFileDownload($course_id,$download)
    {
        ...
        if($download == "checked"){
        	$zip = new ZipArchive;
	        $fileName = storage_path('private/Assessment_Final/PastYear/'.$name.'.zip');
                    ...
                    foreach($result_list as $rl_row){
		                ...
		                foreach ($files as $key => $value) {
		                    ...
		                    if($rl_row->ass_fx_document == $relativeNameInZipFile){
		                        ...
		                        if($rl_row->ass_fx_type=="Question"){
		                            $zip->addFile($value,$course[0]->semester_name."/".$ass_row->assessment_name."/Question/".$rl_row->ass_fx_name.'.'.$ext[1]);
		                        }else{
		                            $zip->addFile($value,$course[0]->semester_name."/".$ass_row->assessment_name."/Solution/".$rl_row->ass_fx_name.'.'.$ext[1]);
		                        } 
		                    }
		                }
		            }
			...
			for($i=1;$i<(count($string)-1);$i++){
				...
				$course = DB::table('courses')
                 ...
                 $zip->addEmptyDir($course[0]->semester_name."/".$AssFinal->assessment_name);
                 ...
                 foreach($result_list as $rl_row){
		            if($rl_row->ass_fx_type=="Question"){
		                $zip->addEmptyDir($course[0]->semester_name."/".$AssFinal->assessment_name."/Question");
		            }else{
		                $zip->addEmptyDir($course[0]->semester_name."/".$AssFinal->assessment_name."/Solution"); 
		            }
		            foreach ($files as $key => $value) {
		                $relativeNameInZipFile = basename($value);
		                if($rl_row->ass_fx_document == $relativeNameInZipFile){
		                    $ext = explode('.',$relativeNameInZipFile);
		                    if($rl_row->ass_fx_type=="Question"){
		                        $zip->addFile($value,$course[0]->semester_name."/".$AssFinal->assessment_name."/Question/".$rl_row->ass_fx_name.'.'.$ext[1]);
		                    }else{
		                        $zip->addFile($value,$course[0]->semester_name."/".$AssFinal->assessment_name."/Solution/".$rl_row->ass_fx_name.'.'.$ext[1]);
		                    } 
		                }
		            }
		        }
				...
				$course = DB::table('courses')
		         ...
		         foreach ($files as $key => $value) {
		            ...
		            if($AssessmentFinal->ass_fx_document == $relativeNameInZipFile){
		                $ext = explode('.',$relativeNameInZipFile);
		                if($AssessmentFinal->ass_fx_type=="Question"){
		                    $zip->addFile($value,$course[0]->semester_name."/".$AssFinal->assessment_name."/Question/".$AssessmentFinal->ass_fx_name.'.'.$ext[1]);
		                }else{
		                    $zip->addFile($value,$course[0]->semester_name."/".$AssFinal->assessment_name."/Solution/".$AssessmentFinal->ass_fx_name.'.'.$ext[1]);
		                } 
		            }
		        }
			}
	                ...
	                foreach($result_list as $rl_row){
			            ...
			            foreach ($files as $key => $value) {
			                ...
			                if($rl_row->ass_fx_document == $relativeNameInZipFile){
			                    $ext = explode('.',$relativeNameInZipFile);
			                    if($rl_row->ass_fx_type=="Question"){
			                        $zip->addFile($value,"Question & Solution/".$p_row->semester_name."/".$ass_row->assessment_name."/Question/".$rl_row->ass_fx_name.'.'.$ext[1]);
			                    }else{
			                        $zip->addFile($value,"Question & Solution/".$p_row->semester_name."/".$ass_row->assessment_name."/Solution/".$rl_row->ass_fx_name.'.'.$ext[1]);
			                    } 
			                }
			            }
			        }
    ...
    public function searchAssessment(Request $request)
    {
        ...
        if($value!=""){
        	$data_name = DB::table('ass_final')
                    ->join('courses','courses.course_id','=','ass_final.course_id')
                    ->join('subjects', 'courses.subject_id', '=', 'subjects.subject_id')
                    ->join('semesters', 'courses.semester', '=', 'semesters.semester_id')
                    ->select('subjects.*','courses.*','semesters.*','ass_final.*')
                    ->where('subjects.subject_id', '=', $subjects[0]->subject_id)
                    ->where('courses.course_id','!=',$course_id)
                    ->Where(function($query) use ($value) {
                        $query->orWhere('ass_final.assessment_name','LIKE','%'.$value.'%')
           	...
           	if(count($data_name)>0) {
           		...
           		foreach($data_name as $ass_row_name){
	                ...
	                $result .= '<input type="checkbox" value="'.$ass_row_name->fx_id.'" class="group_q group_download">';
	                $result .= '</div>';
	                $result .= '<a href="/PastYear/FinalAssessment/'.$course_id.'/list/'.$ass_row_name->fx_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
           		...
           		$data_word = DB::table('assessment_final')
                    ...
                    ->join('subjects', 'courses.subject_id', '=', 'subjects.subject_id')
                    ->join('semesters', 'courses.semester', '=', 'semesters.semester_id')
                    ->select('subjects.*','courses.*','semesters.*','ass_final.*','assessment_final.*')
                    ->where('subjects.subject_id', '=', $subjects[0]->subject_id)
                    ->where('courses.course_id','!=',$course_id)
                    ->Where(function($query) use ($value) {
                ...
                if(count($data_word)>0) {
                	...
                	foreach($data_word as $ass_row_word){
                        ...
                        $result .= '</div>';
                        $result .= '<a href="/images/final_assessment/'.$ass_row_word->ass_fx_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$ass_row_word->semester_name.' : '.$ass_row_word->assessment_name.' / '.$ass_row_word->ass_fx_type.' / '.$ass_row_word->ass_fx_name.' <br> <a href='."/final_assessment/view/whole_paper/".$ass_row_word->fx_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                        $result .= '<div class="col-1" style="position: relative;top: -2px;">';
        	...
        	$previous_semester = DB::table('courses')
           	...
           	foreach($previous_semester as $row){
	            ...
	            $result .= '</div>';
	            $result .= '<a href="/PastYear/FinalAssessment/'.$course_id.'/assessment_name/'.$row->course_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
    ...
    public function downloadFiles($ass_fx_id){
        ...
        if(count($course)>0){
            ...
            return Storage::disk('private')->download('Assessment_Final/'.$assessment_final->ass_fx_document, $question."_".$assessment_final->ass_fx_type."_".$assessment_final->ass_fx_name.'.'.$ext[1]);
        }else{
    ...
    public function searchAssessmentName(Request $request)
    {
        ...
        if($value!=""){
            ...
            foreach($ass_final as $row){
	            ...
	            $result .= '</div>';
	            $result .= '<a href="/PastYear/FinalAssessment/'.$original_id.'/list/'.$row->fx_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
	            $result .= '<img src="'.url('image/file.png').'" width="20px" height="25px"/>';
	            $result .= '</div>';
	            $result .= '<div class="col-10" id="assessment_name">';
    ...
    public function PastYearAssessmentList($id,$fx_id)
    {
    	$user_id       = auth()->user()->user_id;
        $staff_dean    = Staff::where('user_id', '=', $user_id)->firstOrFail();
        $faculty_id    = $staff_dean->faculty_id;
                ...
                foreach($group_list as $row_group){
                    ...
                    foreach($assessment_final as $row){
                        if($row_group->ass_fx_type == $row->ass_fx_type){
                            ...
                            $result .= '</div>';
                            $result .= '<a href="/images/final_assessment/'.$row->ass_fx_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$course[0]->semester_name.' : '.$AssFinal->assessment_name.' / '.$row_group->ass_fx_type.' / '.$row->ass_fx_name.' <br> <a href='."/final_assessment/view/whole_paper/".$row->fx_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                              $result .= '<img src="'.url('image/img_icon.png').'" width="25px" height="20px"/>';
                            $result .= '</div>';
                            $result .= '<div class="col-10" id="course_name">';
   	...
   	public function zipFileDownloadList($fx_id,$download)
    {
        ...
        $name = $subjects[0]->semester_name." ".$AssFinal->assessment_name." ( ".$subjects[0]->subject_code." )";
        $zip = new ZipArchive;
        $fileName = storage_path('private/Assessment_Final/PastYear/'.$name.'.zip');
        $zip->open($fileName, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
        $files = File::files(storage_path('/private/Assessment_Final/'));
    ...
    public function searchAssessmentResult(Request $request)
    {
        ...
        if($value!=""){
            $batch_list = DB::table('assessment_final_result')
			                 ...
			                 ->join('semesters', 'courses.semester', '=', 'semesters.semester_id')
			                 ->select('assessment_final_result.*','students.*','courses.*','semesters.*')
			                 ->where('courses.course_id','!=',$course_id)
			                 ->Where(function($query) use ($value) {
			...
			if(count($batch_list)>0) {
				...
				foreach($batch_list as $rs_row){
		            ...
		            $result .= '</div>';
		            $result .= '<a href="/PastYear/FinalSampleResult/'.$course_id.'/previous/'.$rs_row->course_id.'/'.$rs_row->batch.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		            $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
		            $result .= '</div>';
		            $result .= '<div class="col-10" id="course_name">';
	           	...
	           	$student_list = DB::table('assessment_final_result')
				                 ...
				                 ->join('courses','assessment_final_result.course_id','=','courses.course_id')
				                 ->join('semesters', 'courses.semester', '=', 'semesters.semester_id')
				                 ->select('assessment_final_result.*','students.*','users.*','courses.*','semesters.*')
				                 ->where('courses.course_id','!=',$course_id)
				                 ->Where(function($query) use ($value) {
				...
				if(count($student_list)>0) {
					...
					foreach($student_list as $stu_row){
			            ...
			            $result .= '</div>';
			            $result .= '<a href="/PastYear/FinalSampleResult/'.$course_id.'/result/'.$stu_row->fxr_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
			            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
			            $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
			            $result .= '</div>';
			            $result .= '<div class="col-10" id="course_name">';
				    ...
				    if(count($submitted_list)>0){
					    ...
					    foreach($submitted_list as $sub_row){
				            ...
				            $result .= '</div>';
				            $result .= '<a href="/PastYear/FinalSampleResult/'.$course_id.'/previous/'.$sub_row->course_id.'/'.$sub_row->submitted_by.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
				            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
				            $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
				            $result .= '</div>';
				            $result .= '<div class="col-10" id="course_name">';
            ...
            foreach($previous_result_semester as $row){
	            ...
	            $result .= '</div>';
	            $result .= '<a href="/PastYear/FinalSampleResult/'.$course_id.'/previous/'.$row->course_id.'/All/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
	            $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
	            $result .= '</div>';
	            $result .= '<div class="col-10" id="course_name">';
    ...
    public function zipFileDownloadResult($course_id,$download)
    {
        ...
        $name = $subjects[0]->subject_name." ( ".$subjects[0]->subject_code." )";
        $zip = new ZipArchive;
	    $fileName = storage_path('private/Final_Result/PastYear/'.$name.'.zip');
	    $zip->open($fileName, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
	    $Resultfiles = File::files(storage_path('/private/Final_Result/'));
        if($download == "checked"){
        	for($i=1;$i<(count($string)-1);$i++){
        ...
        if($value!=""){
            ...
            if(count($result_list)>0) {
                ...
                foreach($result_list as $row){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/PastYear/FinalSampleResult/'.$id.'/result/'.$row->fxr_id.'" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
            ...
            $result .= '<div class="row col-md-12" id="lecturer"  style="margin:12px 0px 0px 0px;padding: 0px 0px 5px 0px;border-bottom:1px solid black;">';
                foreach($lecturer_result as $row){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/PastYear/sampleResult/'.$id.'/result/'.$row->fxr_id.'" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
                    $result .= '<div class="col-12 row" style="padding:10px 10px 10px 0px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;padding-left: 2px;">';
                    $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
                    $result .= '</div>';
            ...
            if(count($student_result)>0) {
                ...
                foreach($student_result as $sow){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/PastYear/sampleResult/'.$id.'/result/'.$sow->fxr_id.'" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
                    $result .= '<div class="col-12 row" style="padding:10px 10px 10px 0px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;padding-left: 2px;">';
                    $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
                    $result .= '</div>';
    ...
    public function zipFileDownloadStudent($course_id,$download)
    {
        ...
        $name = "Final Sample Result ( ".$subjects[0]->semester_name." )";
        $zip = new ZipArchive;
        $fileName = storage_path('private/Final_Result/PastYear/'.$name.'.zip');
        $zip->open($fileName, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
    ...
    public function zipFileDownloadDocument($student_id,$course_id,$download)
    {
        ...
        $name = "Final Exam Result ( ".$student_id." )";
        $zip = new ZipArchive;
        $fileName = storage_path('private/Final_Result/PastYear/'.$name.'.zip');
        $zip->open($fileName, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
        $files = File::files(storage_path('/private/Final_Result/'));

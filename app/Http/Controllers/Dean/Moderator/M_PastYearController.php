...
class M_PastYearController extends Controller
{
    public function PastYearAssessment($id)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYear.viewAssessment',compact('course','previous_semester'));
        }else{
        ...
        if($value!=""){
        	$data_name = DB::table('assessments')
           	...
           	if(count($data_name)>0) {
           		...
           		foreach($data_name as $ass_row_name){
	                ...
	                $result .= '</div>';
	                $result .= '<a href="'.$character.'/Moderator/PastYear/assessment/'.$course_id.'/list/'.$ass_row_name->ass_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
        	...
        	$previous_semester = DB::table('courses')
           	...
           	foreach($previous_semester as $row){
	            ...
	            $result .= '</div>';
	            $result .= '<a href="'.$character.'/Moderator/PastYear/assessment/'.$course_id.'/assessment_name/'.$row->course_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
    ...
    public function PastYearAssessmentName($id,$course_id)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYear.viewAssessmentName',compact('course','previous','sample_stored','id'));
        }else{
            return redirect()->back();
        }
    }
    ...
    public function PastYearAssessmentList($id,$ass_id)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYear.viewAssessmentList',compact('course','assessments','previous','assessment_list','group_list'));
        }else{
            return redirect()->back();
        }
    }
                ...
                foreach($group_list as $row_group){
                    ...
                    foreach($assessment_list as $row){
                        if($row_group->ass_type == $row->ass_type){
                            ...
                            $result .= '</div>';
                            $result .= '<a href="'.$character.'/Moderator/PastYear/images/assessment/'.$course_id."-".$row->ass_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$previous[0]->semester_name.' : '.$assessments->sample_stored.' / '.$row_group->ass_type.' / '.$row->ass_name.' <br> <a href='.$character."/Moderator/PastYear/assessment/view/whole_paper/".$course_id."-".$row->ass_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                              $result .= '<img src="'.url('image/img_icon.png').'" width="25px" height="20px"/>';
                            $result .= '</div>';
                            $result .= '<div class="col-10" id="course_name">';
           	...
           	if(count($data_name)>0) {
			    ...
			    if(count($batch_list)>0) {
				    ...
				    foreach($batch_list as $rs_row){
		                ...
		                $result .= '</div>';
		                $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$course_id.'/previous/'.$rs_row->course_id.'/'.$rs_row->batch.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		                $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		                $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
		                $result .= '</div>';
		                $result .= '<div class="col-10" id="course_name">';
				    ...
				    if(count($student_list)>0) {
					    ...
					    foreach($student_list as $stu_row){
			                ...
			                $result .= '</div>';
			                $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$course_id.'/previous/'.$stu_row->course_id.'/'.$stu_row->student_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
			                $result .= '<div class="col-1" style="position: relative;top: -2px;">';
			                $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
			                $result .= '</div>';
			                $result .= '<div class="col-10" id="course_name">';
				        ...
				        if(count($submitted_list)>0){
					    	...
					    	foreach($submitted_list as $sub_row){
				                ...
				                $result .= '</div>';
				                $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$course_id.'/previous/'.$sub_row->course_id.'/'.$sub_row->submitted_by.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
				                $result .= '<div class="col-1" style="position: relative;top: -2px;">';
				                $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
				                $result .= '</div>';
				                $result .= '<div class="col-10" id="course_name">';
    ...
    public function PastYearResultAssessmentList($id,$course_id,$search)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYear.viewSRAssessmentList',compact('search','id','course','previous','sample_stored'));
        }else{
            return redirect()->back();
        }
    }
        ...
        if($value!=""){
			...
			if(count($batch_list)>0){
				...
				foreach($batch_list as $row){
		            ...
		            $result .= '</div>';
		            $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$original_id.'/name/'.$row->ass_id.'/'.$row->batch.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		            $result .= '<img src="'.url('image/file.png').'" width="20px" height="25px"/>';
		            $result .= '</div>';
		            $result .= '<div class="col-10" id="assessment_name">';
			    ...
			    if(count($student_list)>0){
				    ...
				    foreach($student_list as $row){
			            ...
			            $result .= '<input type="checkbox" value="'.$row->ar_stu_id.'" class="group_download">';
			            $result .= '</div>';
			            $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$original_id.'/result/'.$row->ar_stu_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
			            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
			            $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
			            $result .= '</div>';
			        ...
			        if(count($submitted_list)>0){
					    ...
					    foreach($submitted_list as $row){
				            ...
				            $result .= '<input type="checkbox" value="'.$row->ar_stu_id.'" class="group_download">';
				            $result .= '</div>';
				            $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$original_id.'/name/'.$row->ass_id.'/'.$row->submitted_by.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
				            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
				            $result .= '<img src="'.url('image/file.png').'" width="20px" height="25px"/>';
				            $result .= '</div>';
				            $result .= '<div class="col-10" id="assessment_name">';
            ...
            foreach($assessments as $row){
		            ...
		            $result .= '<input type="checkbox" value="'.$row->ass_id.'" class="group_download">';
		            $result .= '</div>';
		            $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$original_id.'/name/'.$row->ass_id.'/All" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		            $result .= '<img src="'.url('image/file.png').'" width="20px" height="25px"/>';
		            $result .= '</div>';
		            $result .= '<div class="col-10" id="assessment_name">';
    ...
    public function PastYearStudentList($id,$ass_id,$search)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYear.viewSRStudentList',compact('search','id','course','lecturer_result','student_result','assessments','previous'));
        }else{
            return redirect()->back();
        }
    }
        ...
        if($value!=""){
            ...
            if(count($result_list)>0) {
                ...
                foreach($result_list as $row){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$course_id.'/result/'.$row->ar_stu_id.'" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
            ...
            if(count($student_result)>0) {
                ...
                foreach($student_result as $sow){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="'.$character.'/Moderator/PastYear/sampleResult/'.$course_id.'/result/'.$sow->ar_stu_id.'" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
                    $result .= '<div class="col-12 row" style="padding:10px 10px 10px 0px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;padding-left: 2px;">';
                    $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
                    $result .= '</div>';
    ...
    public function view_wholePaper($ass_id)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.Assessment.viewWholePaper', compact('assessments','assessment_list','question','string'));
        }else{
            return redirect()->back();
        }
    }
    ...
    public function view_wholePaperResult($ar_stu_id)
    {
        ...
        $assessment_result_list = DB::table('assessment_result_students')
                                ...
                                ->get();
        if(count($course)>0){
            return view('dean.Moderator.AssessmentResult.viewWholePaper', compact('assessment_result_list','assessments','checkARID','submitted_by','string'));
        }else{
            return redirect()->back();
        }
    }

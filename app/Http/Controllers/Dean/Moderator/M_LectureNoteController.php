...
class M_LectureNoteController extends Controller
{
	public function ModeratorLectureNote($id)
	{
        ...
        if(count($course)>0){
            return view('dean.Moderator.Lecture_Note.M_Lecture_note',compact('course','lecture_note','all_note'));
        }else{
            return redirect()->back();
        }
	}

	public function searchModeratorLN(Request $request)
	{
	        ...
	        if(count($lecture_note)>0) {
	        	foreach($lecture_note as $row){
		        	...
		        	if($row->note_type=="folder"){
                        ...
                        $result .= '</div>';
                        $result .= '<a href="'.$character.'/Moderator/lectureNote/folder/'.$row->ln_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                        ...
                        if(($ext[1] == "pdf")||($ext[1] == "docx")||($ext[1] == "xlsx")||($ext[1] == "pptx")||($ext[1] == "ppt")){
                            ...
                            $result .= '<input type="checkbox" value="'.$row->ln_id.'" class="group_download_list">';
                            $result .= '</div>';
                            $result .= '<a href="'.$character.'/Moderator/lectureNote/download/'.$row->ln_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                            if($ext[1]=="pdf"){
                            ...
                            $result .= '<input type="checkbox" value="'.$row->ln_id.'" class="group_download_list">';
                            $result .= '</div>';
                            $result .= '<a href="'.$character.'/Moderator/images/lectureNote/'.$row->ln_id.'/'.$row->note.'" data-toggle="lightbox" data-gallery="example-gallery_student" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$row->note_name.$semester_name.'">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
	...
	public function ModeratorLNFolderView($folder_id)
	{
		$user_id       = auth()->user()->user_id;
        $staff_dean    = Staff::where('user_id', '=', $user_id)->firstOrFail();
        $faculty_id    = $staff_dean->faculty_id;
        ...
        if(count($course)>0){
        	return view('dean.Moderator.Lecture_Note.M_LNFolderView', compact('course','note_place','lecture_note','lecture_note_list','all_note','data'));
        }else{
            return redirect()->back();
        }
	}
    ...
    public function downloadLN($id)
    {
        ...
        $moderator = $course->moderator;

        $user_id    = auth()->user()->user_id;
        $checkid    = Staff::where('user_id', '=', $user_id)->firstOrFail();
        $staff_id   = $checkid->id;

        if($moderator == $staff_id){
            $ext = "";

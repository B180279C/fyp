...
class M_PastYearNoteController extends Controller
{
	public function PastYearNote($id)
    {
        ...
        if(count($course)>0){
            return view('dean.Moderator.PastYearNote.viewPYNote',compact('course','previous_semester'));
        }else{
	        ...
	        if(count($lecture_note)>0) {
	        	foreach($lecture_note as $row){
		        	...
		        	if($row->note_type=="folder"){
	                    ...
	                    $result .= '</div>';
	                    $result .= '<a href="'.$character.'/Moderator/PastYearNote/'.$course_id.'/folder/'.$row->ln_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                        ...
                        if(($ext[1] == "pdf")||($ext[1] == "docx")||($ext[1] == "xlsx")||($ext[1] == "pptx")){
		                    ...
		                    $result .= '<input type="checkbox" value="'.$row->ln_id.'" class="group_download">';
		                    $result .= '</div>';
		                    $result .= '<a href="'.$character.'/Moderator/PastYear/lectureNote/download/'.$course_id."-".$row->ln_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		                    $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		                    if($ext[1]=="pdf"){
	            ...
	            if(count($previous_semester)>0) {
	            	foreach($previous_semester as $row){
		                ...
		                $result .= '</div>';
		                $result .= '<a href="'.$character.'/Moderator/PastYearNote/'.$course_id.'/course/'.$row->course_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
		                $result .= '<div class="col-1" style="position: relative;top: -2px;">';
		                $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
		                $result .= '</div>';
    ...
    public function PastYearNoteViewIn($id,$view,$view_id)
    {
        ...
        if(count($course)>0){
        	if($view=="course"){
            	return view('dean.Moderator.PastYearNote.viewPYNoteFolder',compact('course','lecture_note','previous','all_note'));
        	}else{
        		return view('dean.Moderator.PastYearNote.viewPYNoteFolder',compact('course','lecture_note','previous','ln','data','all_note'));
        	}
        }else{
            return redirect()->back();
        }
    ...
    public function downloadLN($id)
    {
        ...
        $course = Course::where('course_id', '=', $ids[0])->firstOrFail();

        ...
        if($value!=""){
        	$data_name = DB::table('ass_final')
           	...
           	if(count($data_name)>0) {
           		...
           		foreach($data_name as $ass_row_name){
	                ...
	                $result .= '</div>';
	                $result .= '<a href="'.$character.'/Reviewer/PastYear/FinalAssessment/'.$course_id.'/list/'.$ass_row_name->fx_id.'/" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
                ...
                if(count($data_word)>0) {
                	...
                	foreach($data_word as $ass_row_word){
                        ...
                        $result .= '</div>';
                        $result .= '<a href="'.$character.'/Reviewer/PastYear/images/final_assessment/'.$course_id."-".$ass_row_word->ass_fx_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$ass_row_word->semester_name.' : '.$ass_row_word->assessment_name.' / '.$ass_row_word->ass_fx_type.' / '.$ass_row_word->ass_fx_name.' <br> <a href='.$character."/Reviewer/PastYear/final_assessment/view/whole_paper/".$course_id."-".$ass_row_word->fx_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                        $result .= '<div class="col-1" style="position: relative;top: -2px;">';
        	...
        	$previous_semester = DB::table('courses')
           	...
           	foreach($previous_semester as $row){
	            ...
	            $result .= '</div>';
	            $result .= '<a href="'.$character.'/Reviewer/PastYear/FinalAssessment/'.$course_id.'/assessment_name/'.$row->course_id.'" id="show_image_link" class="col-11 row" style="padding:10px 0px;margin-left:-10px;color:#0d2f81;border:0px solid black;">';
	            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                ...
                foreach($group_list as $row_group){
                    ...
                    foreach($assessment_final as $row){
                        if($row_group->ass_fx_type == $row->ass_fx_type){
                            ...
                            $result .= '</div>';
                            $result .= '<a href="'.$character.'/Reviewer/PastYear/images/final_assessment/'.$original_id."-".$row->ass_fx_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$previous[0]->semester_name.' : '.$AssFinal->assessment_name.' / '.$row_group->ass_fx_type.' / '.$row->ass_fx_name.' <br> <a href='.$character."/Reviewer/PastYear/final_assessment/view/whole_paper/".$original_id."-".$row->fx_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';
                              $result .= '<img src="'.url('image/img_icon.png').'" width="25px" height="20px"/>';
                            $result .= '</div>';
                            $result .= '<div class="col-10" id="course_name">';

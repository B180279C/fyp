...
class D_AssessmentResultController extends Controller
{
    public function viewAssessmentStudentResult($id,$question)
    {
        ...
        if((count($course)>0)&&(count($assessments)>0)){
            return view('dean.Dean.AssessmentResult.viewAssessmentStudentResult',compact('course','question','assessments'));
        }else{
    ...
    public function viewstudentlist($ass_id)
    {
        ...
        if(count($course)>0){
            return view('dean.Dean.AssessmentResult.viewStudentResult',compact('course','assessments','lecturer_result','student_result'));
        }else{
            return redirect()->back();
        }
    }
    ...
    public function viewStudentResult($ar_stu_id)
    {
        ...
        if(count($course)>0){
            return view('dean.Dean.AssessmentResult.viewResultList',compact('course','assessments','assessment_result_student','lecturer_result','student_result'));
        }else{
            return redirect()->back();
        }
    }
        ...
        if($value!=""){
            ...
            if(count($assessment_results)>0) {
                foreach($assessment_results as $row){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/Dean/AssessmentResult/studentResult/'.$row->ass_id.'/" class="col-11 row" style="padding:10px 0px;margin-left:0px;color:#0d2f81;border:0px solid black;" id="show_image_link">';
        ...
        if($value!=""){
            ...
            $result .= '<div class="row col-md-12" id="lecturer"  style="margin:12px 0px 0px 0px;padding: 0px 0px 5px 0px;border-bottom:1px solid black;">';
                foreach($result_list as $row){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/Dean/AssessmentResult/view/student/'.$row->ar_stu_id.'/" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
            ...
            if(count($student_result)>0) {
                ...
                foreach($student_result as $sow){
                    ...
                    $result .= '</div>';
                    $result .= '<a href="/Dean/AssessmentResult/view/student/'.$sow->ar_stu_id.'/" class="col-11 row align-self-center" id="show_image_link" style="margin-left:0px;border:0px solid black;">';
                    $result .= '<div class="col-12 row" style="padding:10px 10px 10px 0px;color:#0d2f81;">';
                    $result .= '<div class="col-1" style="position: relative;top: -2px;padding-left: 2px;">';
                    $result .= '<img src="'.url('image/folder2.png').'" width="25px" height="25px"/>';
                    $result .= '</div>';

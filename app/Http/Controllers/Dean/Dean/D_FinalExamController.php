  ...
  public function viewFinalExamination($id)
    {
      ...
      if(auth()->user()->position=="Dean"){
        ...
        if(count($course)>0){
            return view('dean.Reviewer.FinalExam.viewFinalExam',compact('course','ass_final','action','moderator_person_name','verified_person_name','approved_person_name','assessment_final'));
        }else{
            return redirect()->back();
        }
    }
    ...
    public function create_final_list($coursework,$id)
    {
      $user_id       = auth()->user()->user_id;
        ...
        if(count($course)>0){
            return view('dean.Reviewer.FinalExam.createFinalList',compact('course','mark','coursework','ass_final','TP_Ass','tp','group_list','assessment_final'));
        }else{
            return redirect()->back();
        }
    }
    ...
    public function create_question($coursework,$id)
    {
      $user_id       = auth()->user()->user_id;
        ...
        if(count($course)>0){
            return view('dean.Reviewer.FinalExam.createFinalQuestion',compact('course','mark','coursework','ass_final','TP_Ass','tp','assessment_final','group_list'));
        }else{
            return redirect()->back();
        }
    }
                ...
                foreach($group_list as $row_group){
                    ...
                    foreach($assessment_list as $row){
                        if($row_group->ass_fx_type == $row->ass_fx_type){
                            ...
                            $result .= '</div>';
                            $result .= '<a href="'.$character.'/Reviewer/images/final_assessment/'.$row->ass_fx_document.'" data-toggle="lightbox" data-gallery="example-gallery" class="col-11 row" style="padding:10px 0px;margin-left:5px;color:#0d2f81;border:0px solid black;" id="show_image_link" data-title="'.$course[0]->semester_name.' : Final / '.$row_group->ass_fx_type.' / '.$row->ass_fx_name.' <br> <a href='.$character."/Reviewer/final_assessment/view/whole_paper/".$row->course_id.' class='."full_question".' target='."_blank".'>Whole paper</a>">';
                            $result .= '<div class="col-1" style="position: relative;top: -2px;">';

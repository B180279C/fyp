...
class TeachingPlanController extends Controller
{
    ...
    if(count($course)>0){
      return view('student.Teaching_Plan.TeachingPlan',compact('course','TP','topic','TP_Ass','TP_CQI'));
    }else{
	...
	public function TPDownload($id){
      ...
      $course    = DB::table('assign_student_course')
                    ...
                    ->join('staffs', 'staffs.id','=','courses.lecturer')
                    ->join('users', 'staffs.user_id', '=' , 'users.user_id')
                    ->select('assign_student_course.*','semesters.*','subjects.*','staffs.*','users.*','programmes.*','courses.*')
                    ->where('assign_student_course.student_id', '=', $student->student_id)
                    ->where('assign_student_course.course_id','=',$id)
                    ->get();
    ...
    //              ->where('staffs.department_id','=',$course[0]->department_id)
    //              ->get();
